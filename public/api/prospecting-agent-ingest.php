...
function prospecting_agent_claim_website_research_request(PDO $pdo,int $ownerId,array $body):array
{
    prospecting_reject_unknown($body,['request_public_id']);$public=prospecting_text($body,'request_public_id',80);$query=$pdo->prepare('select * from prospecting_website_research_requests where owner_id=? and public_id=? for update');$query->execute([$ownerId,$public]);$request=$query->fetch();if(!$request)throw new ProspectingError('The website research request could not be found.',404);
    if($request['status']==='CLAIMED'){ $run=prospecting_agent_fetch_run($pdo,$ownerId,(string)$pdo->query('select public_id from prospecting_agent_runs where id='.(int)$request['agent_run_id'])->fetchColumn(),true);return ['request_public_id'=>$public,'run'=>prospecting_agent_run_row($run),'idempotent_replay'=>true]; }
    if($request['status']!=='PENDING')throw new ProspectingError('Only pending website research requests can be claimed.',409);
    $mission=prospecting_find($pdo,'prospecting_missions',$ownerId,(string)$pdo->query('select public_id from prospecting_missions where id='.(int)$request['mission_id'])->fetchColumn(),true);$prospect=prospecting_find($pdo,'prospecting_prospects',$ownerId,(string)$pdo->query('select public_id from prospecting_prospects where id='.(int)$request['prospect_id'])->fetchColumn(),true);if($mission['status']!=='RUNNING'||$prospect['status']!=='RESEARCHING')throw new ProspectingError('The request is no longer eligible for research.',409);
    $runPublic=prospecting_public_id('agent-run');$pdo->prepare("insert into prospecting_agent_runs(public_id,owner_id,mission_id,prospect_id,status,started_at) values(?,?,?,?, 'RUNNING',utc_timestamp())")->execute([$runPublic,$ownerId,$request['mission_id'],$request['prospect_id']]);$run=prospecting_agent_fetch_run($pdo,$ownerId,$runPublic,true);$pdo->prepare("update prospecting_website_research_requests set status='CLAIMED',agent_run_id=?,claimed_at=utc_timestamp() where id=?")->execute([$run['id'],$request['id']]);return ['request_public_id'=>$public,'run'=>prospecting_agent_run_row($run),'idempotent_replay'=>false];
}

function prospecting_agent_persist_website_research(PDO $pdo,int $ownerId,array $body):array
{
    prospecting_reject_unknown($body,['mission_public_id','prospect_public_id','run_public_id','request_public_id','idempotency_key','requested_url','canonical_host','status','stop_reason','pages_attempted','pages_succeeded','started_at','completed_at','evidence','events']);
    ...
    $requestPublic=prospecting_text($body,'request_public_id',80,false);$request=null;if($requestPublic!==null){$rq=$pdo->prepare("select * from prospecting_website_research_requests where owner_id=? and public_id=? for update");$rq->execute([$ownerId,$requestPublic]);$request=$rq->fetch();if(!$request||(int)$request['mission_id']!==(int)$mission['id']||(int)$request['prospect_id']!==(int)$prospect['id']||(int)$request['agent_run_id']!==(int)$run['id']||$request['status']!=='CLAIMED')throw new ProspectingError('The research request must be claimed by this run.',422);}
    ...
    if($request!==null){$requestStatus=['COMPLETED'=>'COMPLETED','PARTIAL'=>'PARTIAL','BLOCKED'=>'BLOCKED','FAILED'=>'FAILED'][$status];$pdo->prepare('update prospecting_website_research_requests set status=?,website_research_id=?,completed_at=utc_timestamp() where id=?')->execute([$requestStatus,$research['id'],$request['id']]);}
...
function prospecting_agent_dispatch(PDO $pdo,int $ownerId,array $config,string $operation,array $payload):array
{
    return match($operation){
        ...
        'CLAIM_WEBSITE_RESEARCH_REQUEST'=>[prospecting_agent_claim_website_research_request($pdo,$ownerId,$payload),200],
