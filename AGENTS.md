...
- Use Hostinger MariaDB through a same-origin PHP API for production Studio data. Keep database credentials exclusively in the ignored server-side `api/config.php`, use HTTP-only sessions and owner-scoped queries, and never connect the browser directly to MariaDB.
